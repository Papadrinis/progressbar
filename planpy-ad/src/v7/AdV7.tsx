...
type Beat = {clip: string; d: number; shot: number; lines: string[]; keys: string[]; tail?: number};
export type RubroV7 = 'barberia' | 'mascotas';

const BEATS_MASCOTAS: Beat[] = [
  {clip: 'mascotas-h1', d: 4.519, shot: 1, lines: ['Tiene un perro enjabonado', 'y el celular sonando.', '¿A cuál atiende?'], keys: ['enjabonado', 'cuál']},
  {clip: 'mascotas-a1', d: 2.769, shot: 2, lines: ['Con las manos mojadas,', 'no puede contestar.'], keys: ['mojadas']},
  {clip: 'mascotas-a2', d: 4.519, shot: 3, lines: ['Y el sábado: tres perros', 'a la misma hora,', 'y uno que nadie anotó.'], keys: ['tres', 'nadie']},
  {clip: 'mascotas-s1', d: 4.127, shot: 4, lines: ['Con PlanPy, los dueños', 'reservan solos,', 'las 24 horas.'], keys: ['solos', '24']},
  {clip: 'recordatorio-s2', d: 2.116, shot: 5, lines: ['Y el recordatorio', 'les llega por WhatsApp.'], keys: ['whatsapp']},
  {clip: 'mascotas-d', d: 2.273, shot: 6, lines: ['Y usted, dedicado', 'a los peludos.'], keys: ['peludos'], tail: 16},
  {clip: 'cta', d: 3.474, shot: 0, lines: [], keys: [], tail: 50},
];

const BEATS_BARBERIA: Beat[] = [
...
export const timelineV7 = (rubro: RubroV7 = 'barberia') => {
  let from = 0;
  return (rubro === 'mascotas' ? BEATS_MASCOTAS : BEATS_BARBERIA).map((b, i) => {
...
export const durationV7 = (rubro: RubroV7) => timelineV7(rubro).reduce((a, b) => a + b.len, 0);
export const DURATION_V7 = durationV7('barberia');
...
const AgendaCard: React.FC<{at: number; cols: string[]; fixed: string[]; online: string[]}> = ({at, cols, fixed, online}) => {
  const frame = useCurrentFrame();
  const {fps} = useVideoConfig();
  ...
  return (
    <div style={{position: 'absolute', left: 60, top: 1060, width: 940, height: 600, transform: `scale(${0.96 * s}) rotate(${(1 - s) * -6}deg)`, transformOrigin: '50% 50%', borderRadius: 22, overflow: 'hidden', boxShadow: '0 40px 80px rgba(0,0,0,0.55)', border: '10px solid #1d1f22'}}>
      ...
      <div style={{position: 'absolute', top: 34, left: 0, right: 0, bottom: 0}}>
        <AgendaDesktop cols={cols} hours={['9 a. m.', '10 a. m.', '11 a. m.', '12 m.', '2 p. m.', '3 p. m.', '4 p. m.', '5 p. m.']} citas={citas} />
      </div>
    </div>
  );
};
...
const Banner: React.FC<{at: number; business: string; text: string}> = ({at, business, text}) => {
  const frame = useCurrentFrame();
  const {fps} = useVideoConfig();
  ...
  return (
    <div style={{position: 'absolute', left: 50, right: 50, top: 1240, padding: '24px 26px', borderRadius: 34, background: 'rgba(255,255,255,0.96)', fontFamily: `${SANS}, ${EMOJI}`, color: '#141414', boxShadow: '0 20px 50px rgba(0,0,0,0.45)', transform: `translateY(${(1 - s) * 200}px) scale(${0.9 + 0.1 * s})`, opacity: s}}>
      <div style={{display: 'flex', gap: 14, alignItems: 'center', marginBottom: 8}}>
        ...
        <div style={{fontSize: 28, fontWeight: 800}}>{business}</div>
        <div style={{marginLeft: 'auto', fontSize: 22, color: '#8a857c'}}>ahora</div>
      </div>
      <div style={{fontSize: 36, lineHeight: 1.25}}>{text}</div>
      <div style={{fontSize: 18, color: '#8a857c', marginTop: 8}}>Datos de ejemplo</div>
    </div>
  );
};

const OverlayBarberia: React.FC<{i: number; lead: number; vf: number}> = ({i, lead, vf}) => {
  ...
  switch (i) {
    ...
    case 3:
      return (
        <>
          <AgendaCard at={30} cols={['Silla 1', 'Silla 2']} fixed={['Andrés · corte', 'Juan · barba', 'Felipe · corte', 'Mateo · fade']} online={['Santi · corte', 'Camilo · barba', 'Nico · corte', 'Leo · fade']} />
          {[0, 1, 2, 3].map((k) => (
    ...
    case 4:
      return (
        <>
          <Banner at={10} business="Barbería El Parche" text="Recordatorio: su corte es mañana a las 5:00 p. m. ✂️" />
...
const OverlayMascotas: React.FC<{i: number; lead: number; vf: number}> = ({i, lead, vf}) => {
  const w = (f: number) => Math.round(lead + vf * f);
  switch (i) {
    case 0:
      return (
        <>
          <Sticker at={w(0.72)} x={540} y={1640} rot={-4} size={88} bg="#ffd400">
            ¿A CUÁL? 🤔
          </Sticker>
          <Sfx at={0} name="splash" volume={0.45} />
          <Sfx at={8} name="vibrate" volume={0.45} />
          <Sfx at={w(0.72)} name="boing" volume={0.45} />
        </>
      );
    case 1:
      return (
        <>
          <Burbujas start={4} every={10} msgs={['¿Cupo para Lola el sábado? 🐩', '¿Cuánto vale el baño?', '¿Me lo motilan también? 🐕', '¿Hay turno mañana?']} />
          <Sfx at={0} name="vibrate" volume={0.4} />
          {[4, 14, 24, 34].map((a) => (
            <Sfx key={a} at={a} name="ping" volume={0.35} />
          ))}
        </>
    ...
    case 2:
      return (
        <>
          <Sticker at={w(0.42)} x={330} y={640} rot={-5} size={60} bg="#ffd400">
            Sábado 10:00 × 3 🐾
          </Sticker>
          <Sticker at={w(0.8)} x={760} y={760} rot={6} size={56} bg="#ff4d4d" color="#fff">
            ¿Y este? 🤔
          </Sticker>
          <Sfx at={w(0.3)} name="bark" volume={0.4} />
          <Sfx at={w(0.42)} name="boing" volume={0.4} />
          <Sfx at={w(0.8)} name="boing" volume={0.45} />
        </>
      );
    case 3:
      return (
        <>
          <AgendaCard at={30} cols={['Baño', 'Corte']} fixed={['Toby · baño', 'Max · corte', 'Kira · baño', 'Coco · corte']} online={['Lola · baño y corte', 'Rocky · baño', 'Luna · corte', 'Simón · baño']} />
          {[0, 1, 2, 3].map((k) => (
            <Sfx key={k} at={42 + k * 11} name="pop" volume={0.45} />
          ))}
          <Sfx at={30} name="whoosh" volume={0.35} />
    ...
    case 4:
      return (
        <>
          <Banner at={10} business="Spa Patitas" text="Recordatorio: el baño de Lola es mañana a las 10:00 a. m. 🐾" />
          <Sfx at={10} name="ping" volume={0.5} />
        </>
      );
    case 5:
      return (
        <>
          <Sticker at={14} x={540} y={1650} rot={-4} size={68}>
            Puro amor peludo 🐶
          </Sticker>
          <Sfx at={4} name="shake" volume={0.5} />
          <Sfx at={14} name="pop" volume={0.5} />
        </>
      );
    default:
...
export const PlanpyAdV7: React.FC<{rubro?: RubroV7}> = ({rubro = 'barberia'}) => {
  const tl = timelineV7(rubro);
  const total = durationV7(rubro);
  const Overlay = rubro === 'mascotas' ? OverlayMascotas : OverlayBarberia;
  const dir = rubro === 'mascotas' ? 'v8' : 'v7';
  return (
